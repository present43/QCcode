...
public class SpuServiceImpl implements SpuService {
    ...
    public void saveGoods(Goods goods) {
        ...
        for (Sku sku : skuList) {
            ...
            //不启用规格的sku处理 即就是一个普通商品 没有sku  如果不进行判断则会导致空指针异常【sku.getSpec()】
            ...
            //sku.getSpec()  {"颜色":"红","机身内存":"64G"}  JSON.parseObject 将json 格式转换成对象
        ...
        //设置商品类型ID  此ID来自于spu三级分类 的ID
        categoryBrand.setCategoryId(spu.getCategory3Id());
        //设置品牌ID 来自于spu的品牌ID
        categoryBrand.setBrandId(spu.getBrandId());
        //如果count==0 （即查询结果不存在，数据库无此数据） 避免重复元素
